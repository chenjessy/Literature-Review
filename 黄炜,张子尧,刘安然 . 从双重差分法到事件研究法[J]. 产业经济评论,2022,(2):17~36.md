...
3. The assumptions for the identification of DID
   ...
   3.1 Parallel trend assumption
    ...
    The thoughts behind DID is natural experiment. The way of group samples doesn't need to be strictly random. What is important is that the trend of outcome variable is independent to the policy shock ($\delta Y^0$ needs to be random across groups, not have to be Y).

$E(\delta Y^0|D=1)=E(\delta Y^0|D=0)$

   If natural experiment condition is satisfied, then it's RCT, DID is not necessary as the comparison between the control group and treatment group is the treatment effect.
