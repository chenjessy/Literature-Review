...
4. Problems need to be paid attention to
   ...
   Control variables in DID have two functions: to satisfy the conditional independence assumption (or there would be an omitted variable problem, e.g., the individual fixed effect and time fixed effect are necessary for panel data) and to decrease the standard errors. There are bad control variables that can bias the estimation of the causal effect. The key rule of whether a control variable is good or not depends on the determination time. If the determination time is after the policy shock, then it may be a bad control variable.
