...
5. DID and event study
   ...
   When using the event study method, choosing the event window becomes a problem. If the window is too wide, there will be selection bias, sample attrition, and an unbalanced sample. There will also be distortions to the effect by other events. The wider the window, the more efficient the estimation is, but the bigger the bias is. So a robustness check by changing the event window is often required (Sun L., Abraham S., “Estimating dynamic treatment effects in event studies with heterogeneous treatment effects”,  Journal of Econometrics, 2020; Borusyak K., Jaravel X., Spiess J., “Revisiting Event Study Designs: Robust and Efficient Estimation”, Papers, 2021.).

   <img width="1019" height="439" alt="Screenshot 2025-09-22 at 2 28 49 PM" src="https://github.com/user-attachments/assets/07b6be40-df94-4840-9741-10407e7c9f31" />

The authors also raised some important problems to be noted:

1) Need to know the implementation time of the policy. Whether the policy is effectively implemented and the policy stakeholders take some actions is also an important question that needs to be figured out before using DID.

2) The exogeneity needed by DID is on the difference level (the same development trend of rich counties and poor counties). Besides, the exogeneity is scale-dependent (Roth J., Sant’Anna P H C., “Efficient estimation for staggered rollout designs”, Papers, 2021.).

3) The test of SUTVA. The spillover effect is an underlying threat of using DID (Lu Y., Wang J., Zhu L., “Place-based policies, creation, and agglomeration economies: Evidence from China’s  economic zone program”, American Economic Journal: Economic Policy, 2019, 11(3): 325-60.). When doing an analysis of spillover effect, selection effect, or agglomeration effect, DID is not good.

4) When doing DID analysis on policy, it's useful to include the cost of implementing the policy, from an aspect of general equilibrium, to assess the effect of the policy.
